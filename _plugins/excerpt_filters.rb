...
module ExcerptFilters
  ...
  def note_word_count(input)
    text = strip_image_captions(strip_footnotes(strip_note_header(input))).gsub(/<[^>]+>/, ' ')
    text.split(/\s+/).reject(&:empty?).size
  end

  # Image captions (generated by image_tag.rb) are descriptive chrome,
  # not prose — drop them from the word count. Operates on rendered HTML.
  def strip_image_captions(input)
    input.to_s.gsub(%r{<span[^>]*class=["'][^"']*\bimage-caption\b[^"']*["'][^>]*>.*?</span>}im, '')
  end

  # Strip the compact image syntax so its alt/caption text doesn't leak
  # into preview excerpts. Excerpts are built from raw markdown (see
  # strip_wikilinks), so this matches ![alt](src "caption"){: ...}, not
  # the rendered HTML.
